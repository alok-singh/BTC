...
const getSQLQuery = () => {
  return `select url from beta_urls`;
};
...
const cacheSSRPage = async (concurrency) => {
  const query = getSQLQuery();
  const result = await executeQuery(query);
  const urls = result.map(item => item.url.replace('www.astroawani.com', 'beta.astroawani.com'));

  while(urls.length) {
    console.log('starting', concurrency);
    const htmlPages = await Promise.all(urls.splice(0, concurrency).map((url => {
...
cacheSSRPage(process.argv[2] || 100);